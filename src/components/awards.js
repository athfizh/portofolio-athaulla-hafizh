...
export function renderAwards(awards) {
  ...
  list.innerHTML = awards.map((award, i) => {
    ...
    return `
    <div class="award-card reveal reveal-delay-${Math.min(i + 1, 5)}"
      ...
      <div>
        ...
        <p style="font-size:0.75rem;color:var(--muted);">${esc(award.date)}</p>
      </div>
        ...
        ${hasImages ? `
          <div style="display:flex;flex-wrap:wrap;gap:0.875rem;">
            ${award.images.map((img) => {
              ...
              const caption = img.caption || `${award.title} (${award.event})`;
              return `
